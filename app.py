...
def download_if_not_exists(file_id, filename):
    """Download file from Google Drive if not already present locally."""
...
# if not os.path.exists("olympics.csv"):
#     gdown.download(url1, "olympics.csv", quiet=False)
# df = pd.read_csv("olympics.csv")
#
# file_id2 = "1iUqj0VerynaLbbzbYRA8PlaWeBuDccnC"
# url2 = f"https://drive.google.com/uc?id={file_id2}"
# if not os.path.exists("noc_regions.csv"):
#     gdown.download(url2, "noc_regions.csv", quiet=False)
...
# st.sidebar.image("src/logo.png", width=150, caption="Olympics Analysis")
st.sidebar.image('src/logo.png')
# st.sidebar.image('https://wallpaper.dog/large/17051429.jpg')
...
if user_menu == 'Country-wise Analysis':
    ...
    pt = helper.country_event_heatmap(df, selected_country)
    if pt.empty:
        st.warning("No data available to display heatmap for " + selected_country)
    else:
        fig, ax = plt.subplots(figsize=(20, 20))
        ax = sns.heatmap(pt, annot=True)
        st.pyplot(fig)
...
if user_menu == 'Athlete wise Analysis':
    ...
    # ax = sns.scatterplot(temp_df['Weight'],y=temp_df['Height'],hue=temp_df['Medal'],style=temp_df['Sex'],s=60)
    st.pyplot(fig)
